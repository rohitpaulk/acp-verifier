...
test.each(registry.agentSlugs)("can list and switch models (%s)", async (slug) => {
  ...
  if (modeOptions.length > 0) {
    check.pass(
      "listing-modes",
      `${agent.name} advertised ${modeOptions.length} mode(s): ${modeOptions.map((o) => o.name).join(", ")}.`,
    );
  } else {
    check.fail("listing-modes", `${agent.name} did not advertise any session modes (e.g. Ask/Build/Plan).`);
  }
  ...
  if (newModeOption.type !== "select" || !newModeOption.options) {
    throw new Error("Expected mode option to be a select option with options after setting new mode");
  }
  ...
  check.pass("switch-mode", `${agent.name} switched mode from ${currentModeValue} to ${modeValueToChangeTo}.`);
